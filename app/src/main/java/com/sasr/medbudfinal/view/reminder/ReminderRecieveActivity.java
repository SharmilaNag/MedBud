...
public class ReminderRecieveActivity extends AppCompatActivity {
    private static final String TAG = "ReminderRecieveActivity";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_reminder_recieve);
        ...
        if (getIntent() != null) {
            getSupportFragmentManager().beginTransaction().replace(R.id.remReceiveContainer,
                    ReminderReceiveFragment.newInstance(getIntent().getIntExtra(ReminderAddFragment.KEY_ID,-1)),
                    "ReminderReceive").commit();
        } else {
            finish();
        }
    }
